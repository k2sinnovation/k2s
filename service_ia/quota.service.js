...
class QuotaService {
  ...
  async getOrCreateQuota(userId) {
    ...
    // Vérifier reset quotidien/mensuel
    quota.checkAndReset();
   ...
   */
  async canUseTokens(userId, amount = 1) {
    const quota = await this.getOrCreateQuota(userId);

    // Enterprise = illimité
    if (quota.currentPlan === 'enterprise') {
      return { allowed: true, remaining: -1, blocked: false, plan: 'enterprise' };
    }
    ...
    return { allowed: true, remaining, blocked: false, plan: quota.currentPlan };
  }

  /**
   * 📉 Décrémenter les tokens
   */
  async useTokens(userId, amount = 1) {
    const quota = await this.getOrCreateQuota(userId);
  ...
  async canSendEmail(userId) {
    ...
    const remaining = quota.maxEmailsPerDay === -1
      ? -1
      : quota.maxEmailsPerDay - quota.emailsSentToday;

    return { allowed: remaining !== 0, remaining, plan: quota.currentPlan };
  }
  ...
  /**
   * 📊 Récupérer stats détaillées pour UI / Flutter
   */
  async getQuotaStats(userId) {
    ...
    return {
      plan: { name: planConfig.name, level: quota.currentPlan },
      tokens: {
